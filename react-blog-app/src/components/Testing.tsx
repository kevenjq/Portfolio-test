...
export default function ExampleTabs() {
  return (
    // this is the whole part of the element, root acts as a section.
    <Tabs.Root
      className="rounded-md border-1 border-gray-800"
      defaultValue="overview"
    >
      {/*
      list is the part where the list item div will be place on
      */}
      <Tabs.List className="relative z-0 flex gap-1 px-1 shadow-[inset_0_-1px] bg-amber-600 shadow-orange-600">
        {/*
        Tab is the list item that will be seen.
        */}
        <Tabs.Tab
        ...
        {/*
        Indicator is the box that shows what list item is active
        */}
        <Tabs.Indicator className="absolute top-1/2 left-0 z-[-1] h-6 w-[var(--active-tab-width)] -translate-y-1/2 translate-x-[var(--active-tab-left)] rounded-sm bg-purple-400 transition-all duration-500 ease-in-out" />
      </Tabs.List>
      {/*
        Panel is the content that will be shown when the list item is clicked.
      */}
      <Tabs.Panel
      ...
      >
        {/*
        OverviewIcon is the function below that will be used to show the icon.
        */}
        <OverviewIcon className="size-30 text-lime-600" />
      </Tabs.Panel>
      <Tabs.Panel
        className="relative flex h-32 items-center justify-center -outline-offset-1 outline-blue-800 focus-visible:rounded-md focus-visible:outline"
        value="projects"
